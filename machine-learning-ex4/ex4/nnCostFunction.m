...
q = sum(sum(Theta1(:,2:size(Theta1,2)).^2)) + sum(sum(Theta2(:,2:size(Theta2,2)).^2));

J = (1/m)*sum(sum((-Y .* log(a_3) - ((1 - Y) .* log(1-a_3))))) + ((lambda/(2 * m)) * q);
...
for t = 1:m
  ...
  g_p = a_2 .* (1-a_2);
  delta_2 = (delta_3 * Theta2) .* g_p;
  Theta1_grad = Theta1_grad + (a_1' * delta_2)(:,2:end)';
